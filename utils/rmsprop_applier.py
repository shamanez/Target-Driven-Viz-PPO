...
class RMSPropApplier(object):
  ...
  def _create_slots_local(self, var_list):
    for v in var_list: #v is each variable in the global network
      # 'val' is Variable's intial value tensor.
      val = tf.constant(1.0, dtype=v.dtype, shape=v.get_shape())  #create global netowrk variables
      self._get_or_make_slot_local(v, val, "rms_l", self._name) #here the self._slots keep the momenetumn and rms to apply for the global netowrk vartiables
      self._zeros_slot_local(v, "momentum_l", self._name)
  

  def _slot_dict_local(self, slot_name):

    named_slots = self._slots_local.get(slot_name, None)
    if named_slots is None:
      named_slots = {}
      self._slots_local[slot_name] = named_slots
    return named_slots

  def _get_or_make_slot_local(self, var, val, slot_name, op_name):

    named_slots = self._slot_dict_local(slot_name)

    if var not in named_slots:
      named_slots[var] = slot_creator.create_slot(var, val, op_name) #rms prop applier
    return named_slots[var]

  def get_slot_local(self, var, name):

    named_slots = self._slots_local.get(name, None) #slots containing only for the master network variable
    if not named_slots:
      return None
    return named_slots.get(var, None)

  def _zeros_slot_local(self, var, slot_name, op_name):
    named_slots = self._slot_dict_local(slot_name)
    if var not in named_slots:
      named_slots[var] = slot_creator.create_zeros_slot(var, op_name)
    return named_slots[var]
  ...
  def _apply_dense_local(self, grad, var):

      rms = self.get_slot_local(var, "rms_l") #Ex - <tf.Variable 'navigation/W_fc/RMSPropApplier:0' shape=(8192, 512) dtype=float32_ref>
      mom = self.get_slot_local(var, "momentum_l") #get the momentrom slots
  ...
  def apply_gradients_local_net(self, var_list, accum_grad_list, name=None):
    update_ops = []
    ...
    with tf.device(self._device):
      with tf.control_dependencies(None): #eager execution is enabled inside this bloke
        self._create_slots_local(var_list) #Create slots for optimize momentumn varaibles and other rms prop variables (This is actually for creating the slots for gradiets in the global network)
      ...
      with tf.name_scope(name, self._name, []) as name: #RMSPropApplier/
        ...
        for var, accum_grad in zip(var_list, accum_grad_list): #get the relevent varabules and gradiets we culculated
          ...
          with tf.name_scope("update_" + var.op.name), tf.device(var.device):
            ...
            update_ops.append(self._apply_dense_local(clipped_accum_grad, var)) #This is to apply the gradietns
